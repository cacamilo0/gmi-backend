...
from app.modules.m4.schemas import (
    AdherenciaResponse,
    AnticoncepcionCreate,
    AnticoncepcionResponse,
    BirthPlanResponse,
...
async def register_birth(db: AsyncSession, data: PartoCreate, staff_id: str) -> PartoResponse:
    existing = await repository.get_parto_by_gestante(db, data.gestante_id)
    if existing:
        raise ValidationException("Ya existe un registro de parto para esta gestante")
    ...
    parto = Parto(
        ...
        uci_materna=data.uci_materna,
        muerte_materna=data.muerte_materna,
        causa_muerte=data.causa_muerte,
        fecha_muerte=data.fecha_muerte,
    )
    parto = await repository.create_parto(db, parto)
    return PartoResponse.model_validate(parto)


async def get_birth_record(db: AsyncSession, gestante_id: str) -> PartoResponse:
    parto = await repository.get_parto_by_gestante(db, gestante_id)
    if parto is None:
        raise NotFoundException("No se encontró registro de parto")
    return PartoResponse.model_validate(parto)


async def update_birth_record(db: AsyncSession, gestante_id: str, data: PartoUpdate, staff_id: str) -> PartoResponse:
    parto = await repository.get_parto_by_gestante(db, gestante_id)
    if parto is None:
        raise NotFoundException("No se encontró registro de parto")

    if data.tipo_parto is not None:
        parto.tipo_parto = data.tipo_parto
    if data.fecha_parto is not None:
        parto.fecha_parto = data.fecha_parto
    if data.complicaciones is not None:
        parto.complicaciones = data.complicaciones
    if data.uci_materna is not None:
        parto.uci_materna = data.uci_materna
    if data.muerte_materna is not None:
        parto.muerte_materna = data.muerte_materna
    if data.causa_muerte is not None:
        parto.causa_muerte = data.causa_muerte
    if data.fecha_muerte is not None:
        parto.fecha_muerte = data.fecha_muerte

    parto = await repository.update_parto(db, parto)
    return PartoResponse.model_validate(parto)


# ---- Recién Nacido ----

async def register_newborn(db: AsyncSession, data: RecienNacidoCreate, staff_id: str) -> RecienNacidoResponse:
    parto = await repository.get_parto_by_id(db, data.parto_id)
    if parto is None:
        raise NotFoundException("Registro de parto no encontrado")
    ...
    rn = RecienNacido(
        ...
        talla_cm=data.talla_cm,
        uci_neonatal=data.uci_neonatal,
        observaciones=data.observaciones,
    )
    rn = await repository.create_recien_nacido(db, rn)
    return RecienNacidoResponse.model_validate(rn)


async def get_newborns(db: AsyncSession, gestante_id: str) -> list[RecienNacidoResponse]:
    parto = await repository.get_parto_by_gestante(db, gestante_id)
    if parto is None:
        return []
    rns = await repository.get_recien_nacidos_by_parto(db, parto.id)
    return [RecienNacidoResponse.model_validate(rn) for rn in rns]


# ---- Puerperio ----

async def register_postpartum(db: AsyncSession, data: PuerperioCreate, staff_id: str) -> PuerperioResponse:
    p = Puerperio(
        gestante_id=data.gestante_id,
    ...
    p = await repository.create_puerperio(db, p)
    return PuerperioResponse.model_validate(p)


async def get_postpartum_history(db: AsyncSession, gestante_id: str) -> list[PuerperioResponse]:
    puerperios = await repository.get_puerperios_by_gestante(db, gestante_id)
    return [PuerperioResponse.model_validate(p) for p in puerperios]


async def get_postpartum_evolution(db: AsyncSession, gestante_id: str) -> list[PuerperioResponse]:
    """Misma fuente que el historial, ordenado asc para mostrar evolución."""
    return await get_postpartum_history(db, gestante_id)


# ---- Anticoncepción Posparto ----

async def register_contraception(db: AsyncSession, data: AnticoncepcionCreate, staff_id: str) -> AnticoncepcionResponse:
    metodo_nombre = None
    if data.metodo_id:
        metodo = await repository.get_metodo_anticonceptivo_by_id(db, data.metodo_id)
        if metodo is None:
            raise NotFoundException("Método anticonceptivo no encontrado en catálogo")
        metodo_nombre = metodo.nombre

    a = AnticoncepcionPosparto(
        gestante_id=data.gestante_id,
        metodo_id=data.metodo_id,
        fecha_aplicacion=data.fecha_aplicacion,
    )
    a = await repository.create_anticoncepcion(db, a)

    return AnticoncepcionResponse(
        id=a.id,
        gestante_id=a.gestante_id,
        metodo_id=a.metodo_id,
...
async def get_contraception(db: AsyncSession, gestante_id: str) -> list[AnticoncepcionResponse]:
    items = await repository.get_anticoncepcion_by_gestante(db, gestante_id)
    result = []
    for a in items:
        metodo_nombre = None
        if a.metodo_id:
            metodo = await repository.get_metodo_anticonceptivo_by_id(db, a.metodo_id)
            metodo_nombre = metodo.nombre if metodo else None
        result.append(AnticoncepcionResponse(
            id=a.id,
            gestante_id=a.gestante_id,
            metodo_id=a.metodo_id,
            metodo_nombre=metodo_nombre,
