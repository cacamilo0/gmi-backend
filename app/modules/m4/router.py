...
# ---- 5. Parto y Puerperio (específicos M4) ----

@router.post("/birth-record", response_model=PartoResponse, status_code=201)
async def register_birth(
    request: PartoCreate,
    staff: UsuarioStaff = Depends(get_current_staff),
    db: AsyncSession = Depends(get_db),
):
    """Registrar parto. Solo staff."""
    return await service.register_birth(db, request, staff.id)


@router.get("/birth-record", response_model=PartoResponse)
async def get_birth_record(
    gestante: Gestante = Depends(get_current_gestante),
    db: AsyncSession = Depends(get_db),
):
    """Consultar registro de parto de la gestante autenticada."""
    return await service.get_birth_record(db, gestante.id)


@router.put("/birth-record", response_model=PartoResponse)
async def update_birth_record(
    request: PartoUpdate,
    staff: UsuarioStaff = Depends(get_current_staff),
    db: AsyncSession = Depends(get_db),
):
    """Actualizar registro de parto. Solo staff."""
    return await service.update_birth_record(db, request.gestante_id, request, staff.id)


@router.post("/newborn", response_model=RecienNacidoResponse, status_code=201)
async def register_newborn(
    request: RecienNacidoCreate,
    staff: UsuarioStaff = Depends(get_current_staff),
    db: AsyncSession = Depends(get_db),
):
    """Registrar recién nacido. Solo staff. Requiere parto_id."""
    return await service.register_newborn(db, request, staff.id)


@router.get("/newborn", response_model=list[RecienNacidoResponse])
async def get_newborns(
    gestante: Gestante = Depends(get_current_gestante),
    db: AsyncSession = Depends(get_db),
):
    """Consultar recién nacidos de la gestante autenticada."""
    return await service.get_newborns(db, gestante.id)


@router.post("/postpartum", response_model=PuerperioResponse, status_code=201)
async def register_postpartum(
    request: PuerperioCreate,
    staff: UsuarioStaff = Depends(get_current_staff),
    db: AsyncSession = Depends(get_db),
):
    """Registrar evaluación posparto. Solo staff."""
    return await service.register_postpartum(db, request, staff.id)


@router.get("/postpartum", response_model=list[PuerperioResponse])
async def get_postpartum_history(
    gestante: Gestante = Depends(get_current_gestante),
    db: AsyncSession = Depends(get_db),
):
    """Historial de evaluaciones posparto."""
    return await service.get_postpartum_history(db, gestante.id)


@router.get("/postpartum-evolution", response_model=list[PuerperioResponse])
async def get_postpartum_evolution(
    gestante: Gestante = Depends(get_current_gestante),
    db: AsyncSession = Depends(get_db),
):
    """Evolución posparto ordenada cronológicamente."""
    return await service.get_postpartum_evolution(db, gestante.id)


@router.post("/contraception", response_model=AnticoncepcionResponse, status_code=201)
async def register_contraception(
    request: AnticoncepcionCreate,
    staff: UsuarioStaff = Depends(get_current_staff),
    db: AsyncSession = Depends(get_db),
):
    """Registrar método anticonceptivo posparto. Solo staff."""
    return await service.register_contraception(db, request, staff.id)


@router.get("/contraception", response_model=list[AnticoncepcionResponse])
async def get_contraception(
    gestante: Gestante = Depends(get_current_gestante),
    db: AsyncSession = Depends(get_db),
):
    """Consultar anticoncepción posparto registrada."""
